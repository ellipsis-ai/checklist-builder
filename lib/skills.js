...
return ellipsis => {
  ...
  function createNewChecklist(name, firstQuestion) {
    return getSkillEditingContext().then(context => {
      ...
      skillData.behaviorVersions = [
        newAction('run-checklist', 'ellipsis.success()', 'Yay!', skillData.actionInputs.map(ea => ea.inputId))
      ];
  ...
  function newAction(name, functionBody, responseTemplate, inputIds) {
    return {
  ...
  function createNewSkill() {
    return fetch(`${baseUrl}/${ellipsis.token}`).then(res => res.json())
  }
  ...
  function getSkillEditingContext(skillId) {
    ...
    return fetch(`${ellipsis.apiBaseUrl}/api/v1/skill_editing_context/${ellipsis.token}${qs}`).then(res => {
      if (res.ok) {
        return res.json()
      } else {
